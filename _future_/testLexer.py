#!/usr/bin/python
########################################################################
# This file was automatically generated by flg version 3.0.0-build.1
########################################################################
...
class testLexerToken(object):
  """A token returned by testLexer.  The minimal token has a token name, value, and location."""
  ...
  def __init__(self, token=None, value=None, location=None):
    ...
    if isinstance(token,testLexerToken):
...
class testLexer(object):
  """The testLexer lexer class.  This contains the tables for the lexer, plus the lexer code itself."""
...
(u'%skip',())
  ...
  def next_token(self):
    """Return the next token from the source.  It will either return a testLexerToken object with a valid token, an %eof token, or an %error token, or it will return None if we have no valid source stream to read from."""
    ...
    while 1:
      ...
      # the next state machine run.
        ...
        token = testLexerToken(token=tkn, value=val, location=loc)
      ...
      # lookahead buffer, then that other stuff caused a syntax error).
        ...
        if len(self.source) == 0:
          token = u'%eof'
          return testLexerToken(token=self.tables[u'%tokenmap'][token], value=token, location=loc)
      ...
      # passed any accept states.  This is a syntax error.
        ...
        loc = self.lookahead[0][0][1]
        token = testLexerToken(token=u'%error', value=val, location=loc)
